...
describe('atlas data utilities', () => {
  ...
  it('does not reserve result slots for weaker tiers ahead of name prefixes', () => {
    const prefixes = Array.from({ length: 9 }, (_, index) => makeRepo({
      full_name: `owner/python-${index}`, name: `python-${index}`, primary_language: 'Unknown',
    }))
    const language = makeRepo({ full_name: 'owner/language', name: 'language', primary_language: 'Python' })
    const metadata = makeRepo({ full_name: 'owner/metadata', name: 'metadata', platform: 'Python' })
    const results = searchRepos([language, metadata, ...prefixes], 'python')
    expect(results).toHaveLength(8)
    expect(results).toEqual(prefixes.slice(0, 8))
    expect(results).not.toContain(language)
    expect(results).not.toContain(metadata)
  })
